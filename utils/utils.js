...
function findProductInCache(productID) {
  for (const i in productsCache) {
    const p = findProduct(productsCache[i], productID);
    if (p) {
...
export async function getProduct(productID) {
  ...
  const p = findProductInCache(productID);
  if (p) {
    return p;
  }

  let offset = productsCache.length * PRODUCTS_REQUEST_LIMIT;
  let json = await loadProducts(PRODUCTS_URL, offset);
  if (!json.data) return;
  ...
  do {
    ...
    const product = findProduct(json.data, productID);
