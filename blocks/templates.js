...
function getAttributionHTML(data) {
  ...
  if (data.author_url && data.author) {
    const attributionName = getAttributionNameHTML(data.author, data.author_url);
    attribution = `
...
export function bylineTemplate(data) {
  ...
  const template = `<div class="attribution">
            ${attributionHTML}
        </div>
...
export function headerTemplate() {
  const template = `<header class="o-Header" data-module="golf-header">
...
export function headerSchoolsIconTemplate(data) {
  const template = `<svg xmlns="http://www.w3.org/2000/svg" width="123" height="34" viewBox="0 0 123 34" fill="none" role="img" aria-label="${data.title}">
